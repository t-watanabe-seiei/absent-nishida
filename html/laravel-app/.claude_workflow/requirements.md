...
## 機能 D: 担任向け生徒・保護者管理の閲覧化 + 管理者による欠席入力

### 現状
- 担任（is_super_admin=false）の生徒一覧に編集・削除ボタンが表示されている
- 担任に保護者管理画面がナビゲーションに表示されない
- 管理者（担任・スーパー管理者ともに）が欠席連絡を入力できない

### 要件

#### D-1: 生徒管理画面（担任向け）
- 担任の生徒一覧で「編集」「削除」ボタンを非表示にする
- 自分の担当クラスの生徒のみ表示（バックエンドは既に実装済み）

#### D-2: 保護者管理画面（担任向け）
- 担任のナビゲーションに「保護者管理」を追加
- 担任は自分の担当クラスの保護者のみ表示（バックエンドにフィルタ追加）
- 担任は「編集」「削除」ボタンを非表示

#### D-3: 管理者による欠席連絡入力
- スーパー管理者・担任ともに欠席連絡を新規登録できる
  - 担任: 自分のクラスの生徒のみ選択可
  - スーパー管理者: 全生徒から選択可
- 管理者作成の欠席連絡は編集・削除可能
- 保護者作成の欠席連絡は変更・削除不可（閲覧のみ）
- absencesテーブルに `is_admin_created` (boolean, default false) カラムを追加して識別

### 成功基準
- 担任が生徒一覧を開くと編集・削除ボタンが表示されない
- 担任がナビから保護者管理にアクセスでき、自クラスの保護者のみ表示される
- 管理者が欠席連絡を登録・編集・削除できる
- 保護者作成の欠席連絡は管理者から変更・削除できない（403エラー）
